...
#define set_LED_GRN_output()		LED_PORT->MODER				|= 1<<(2*LED_GRN)	//Defines command to set green LED to output mode
#define set_LED_GRN_pushPull()	LED_PORT->OTYPER 			|= 0<<(1*LED_GRN)		//Defines command to set green LED to push pull output mode
#define set_LED_GRN_lowspeed()	LED_PORT->OSPEEDR 		|= 0<<(2*LED_GRN)	//Defines command to set green LED to 2MHz (low speed) operation to save power
#define set_LED_GRN_pull_up()		LED_PORT ->PUPDR 			|= 0<<(2*LED_GRN)	//Defines command to set green LED to internal pull up configuration
